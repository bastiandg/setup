...
download_url="$(jq -r '.assets[] | select(.name | endswith("linux-x86_64.tar.gz")) | .browser_download_url' <<<"$response")"

versioncheck() {
  set +e
  localversion="$(nvim --version 2>/dev/null | grep -oP '(?<=NVIM ).*(?=$)' || true)"
  set -e
  remoteversion="$(jq -r '.tag_name' <<<"$response" || true)"
...
sudo mv nvim-linux-x86_64/ "$INSTALL_PATH"
sudo ln -sf "${INSTALL_PATH}/bin/nvim" /usr/local/bin/nvim
